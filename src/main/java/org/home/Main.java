 ...
 * выделить из них требуемые параметры. Если форматы данных не
 * совпадают, нужно бросить исключение, соответствующее типу
 * проблемы. Можно использовать встроенные типы java и создать
 * свои. Исключение должно быть корректно обработано, пользователю
 ...
 * должно быть корректно обработано, пользователь должен увидеть
 * стектрейс ошибки.
 */
...
public class Main {
    public static void main(String[] args) {
        ...
        InConsole inConsole = new InConsole();
        Person person;
        System.out.println(inConsole.infoInputData());
        try {
            person = inConsole.inputData();
            System.out.printf(person.toString());
            inConsole.saveFileData(person);
        } catch (Exception e) {
